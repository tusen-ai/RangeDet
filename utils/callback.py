...
class Speedometer(object):
    ...
    def __call__(self, param):
        ...
        tb_recorder = param.locals['tb_recorder']
        curr_step = param.locals['curr_step']
        if not isinstance(param.eval_metric, dict):
        ...
        if self.init:
            if count % self.frequent == 0:
                ...
                if param.eval_metric is not None:
                    name, value = param.eval_metric.get()
                    s = "Epoch[%d] Batch [%d] Speed: %.2f samples/sec lr: %.6f\tTrain-" % (
                        param.epoch, count, speed, param.locals['current_lr'])
                ...
                if tb_recorder is not None:
                    for k, v in metric_dict.items():
                        k = k.replace("_output", "")
                        try:
                            tb_recorder.add_scalar(k, float(v), curr_step)
                        except:
                            pass
        else:
            self.init = True
