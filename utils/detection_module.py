...
from utils.local_sync import sync_params_local
...
class DetModule(BaseModule):
    ...
    def load_checkpoint_and_states(self, prefix, epoch, load_optimizer_states=False):
        """Creates a model from previously saved checkpoint.
        ...
        """
        sym, args, auxs = load_checkpoint(prefix, epoch)
        self._arg_params = args
        self._aux_params = auxs
    ...
    def clip_global_norm(self, arrays, max_norm, check_isfinite=True):
        ...
        total_norm = nd.add_n(*[_norm(arr[0]).as_in_context(ctx) for arr in arrays])
    ...
    def update(self):
        ...
        if self._update_on_kvstore:
            ...
            # self.clip_global_norm(self._exec_group.grad_arrays, 1.0)
            # for i, pair in enumerate(zip(self._exec_group.param_arrays, self._exec_group.grad_arrays)):
            ...
            #         w, g = p
            #         #print(self._exec_group.param_names[index], nd.norm(w).asnumpy()[0], nd.norm(g).asnumpy()[0])
    ...
    def fit(self, train_data, eval_data=None, eval_metric='acc',
            ...
            eval_every_n_peoch=1, rank=0, hvd=None, save_path=None, sync_flag=False,
            tb_recorder=None):  # Use tensor board.
        ...
        # self.save_checkpoint('init_pp',0)
        ...
        # training loop
        ################################################################################
        curr_step = int(len(train_data) / train_data.batch_size) * begin_epoch
        for epoch in range(begin_epoch, num_epoch):
            ...
            if sync_flag:
                # sync aux params across host node
                if kvstore is not None and kvstore.num_workers > 1:
                    logging.info(
                        "TODO: sync arg, aux for kv('dist_sync') training, aux(mean, var) on different nodes aren't equal currently, sync by local params file")
                    sync_params_local(kvstore, epoch, save_path, arg_params, aux_params)
                elif hvd is not None and hvd.size() > 1:
                    # for horovod training to sync aux parameters, such as: moving var/mean
                    logging.info("sync arg, aux for horovod training")
            ...
            if epoch_end_callback is not None:
                if (sync_flag == True and rank == 0) or (sync_flag == False and rank % 8 == 0):
                    for callback in _as_list(epoch_end_callback):
                        callback(epoch, self.symbol, arg_params, aux_params)

            if eval_data and (epoch + 1) % eval_every_n_peoch == 0:
